...
password.addEventListener('input', () => {
        ...
        if (strength <= 2) {
                ...
                strengthMsg.innerHTML = 'Lemah Banget Password Lu Kontol.'
        } else if (strength >= 2 && strength <= 4) {
                indicator.classList.remove('weak');
                indicator.classList.add('medium');
                indicator.classList.remove('strong');
                strengthMsg.innerHTML = 'Udah Lumayan, Ketatin Lagi Lah.'
        } else {
                indicator.classList.remove('weak');
                indicator.classList.remove('medium');
                indicator.classList.add('strong');
                strengthMsg.innerHTML = 'Buset Inimah Kuat Bet Bjirr.'
        }
})